# SHIP - PR 2: Today cards stop claiming moves they do not make.
# Branch: fix/card-claim-invariant-pr2   Base: main (PR 1 is already live)
#
#   .\scripts\deploy\ship-pr2-card-claims.ps1                 # full run, prompts before merging
#   .\scripts\deploy\ship-pr2-card-claims.ps1 -AutoMerge      # no prompt once CI is green
#   .\scripts\deploy\ship-pr2-card-claims.ps1 -SkipTests      # only if you JUST ran them
#   .\scripts\deploy\ship-pr2-card-claims.ps1 -MergeOnly      # resume after CI went green
#
...
#
# ---------------------------------------------------------------------------
# WHAT IS SHIPPING, AND WHAT TO EXPECT
#
# Unlike PR 1, this one CHANGES WHAT YOU SEE. The three contradictory war-room
# cards are the point:
#
#   before  Add to SCHD - 4,708
#           "takes Equities from 97% toward your 80% target"
#           "Moves Equities ~22% closer to target"
#           funded by selling TQQQ and SOXL, both equities
#
#   after   Swap into QUAL - 25,720
#           "Does not move your Equities weight (97%). Swaps which equities
#            you hold."
#           funded by selling SCHD and TQQQ; SOXL is untouched because it is a
#           sleeve holding
#
# Expect FEWER cards overall. Three classes of card now refuse to render:
#   1. a buy into an asset class that is already at or over its target, unless
#      it presents itself as a swap
#   2. a buy that would push the book further from the objective's target mix
#   3. a card the book cannot pay for once the earlier cards have taken their
#      share of the same shares and cash
# Fewer cards is the fix, not a regression.
#
# Funding also sells less and sells differently: sleeve holdings are off-limits,
# and a class at or under its target is no longer a funding source while
...
param(
    [string]$Branch   = 'fix/card-claim-invariant-pr2',
...
$Housekeeping = @('scripts/deploy/ship-pr2-card-claims.ps1', 'STATUS.md')
...
$pr1 = git log origin/main --oneline --grep 'funded-buy narration structurally honest' -1
if (-not $pr1) {
    Die 'PR 1 is not on origin/main. Ship it first: scripts\deploy\ship-pr1-card-claims.ps1'
}
Ok "PR 1 is on main: $pr1"
...
if (-not $MergeOnly) {
    ...
    $onHead = git log HEAD --oneline --grep 'funded-buy narration structurally honest' -1
    if (-not $onHead) {
        Die 'this branch does not contain PR 1. Branch off main once PR 1 is merged.'
    }
    Ok 'PR 1 is in this branch history'
}
...
if (-not $SkipTests -and -not $MergeOnly) {
    ...
    Ok 'pytest green (expect 789 passed, 4 skipped; PR 1 baseline was 759)'
...
if (-not $SkipPr) {
    ...
    if ($LASTEXITCODE -eq 0 -and $existing -and $existing.Trim().StartsWith('{')) {
        ...
        gh pr create --base main --head $Branch --title 'Today cards stop claiming moves they do not make (PR 2: consumers)' --body-file $bodyPath
...
git merge --no-ff $Branch -m "Merge PR 2: Today cards stop claiming moves they do not make"
...
Write-Host ''
Write-Host '  OPEN TODAY. This is the one to actually look at:' -ForegroundColor Cyan
Write-Host '    - no card should say "closer to target" while buying a class' -ForegroundColor Gray
Write-Host '      that is already over it' -ForegroundColor Gray
Write-Host '    - an equity buy funded by equity sales should be titled' -ForegroundColor Gray
Write-Host '      "Swap into X" and say "Does not move your Equities weight"' -ForegroundColor Gray
Write-Host '    - no card should say "still leaves N short"' -ForegroundColor Gray
Write-Host '    - no card should propose selling a sleeve holding' -ForegroundColor Gray
Write-Host '    - expect FEWER cards than before; that is the fix' -ForegroundColor Gray
Write-Host ''
Write-Host '  Known limitation, deliberate (see reserve_funding_across):' -ForegroundColor Yellow
Write-Host '    a trim card and a funded buy can still both plan to sell the same' -ForegroundColor Gray
Write-Host '    shares. A trim converts shares to cash rather than consuming them,' -ForegroundColor Gray
Write-Host '    so modelling it means re-sourcing the buy from the proceeds.' -ForegroundColor Gray
Write-Host '    Left for a follow-up rather than half-modelled here.' -ForegroundColor Gray
Write-Host ''
